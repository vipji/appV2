...
import { ChevronRight, Star, Code } from 'lucide-react';
...
const EnhancedProblems = () => {
  ...
  const handleOpenEditor = (problem: any) => {
            ...
            <div className="relative px-6 sm:p-24 w-full z-10">
              <div className="text-center space-y-4 mb-16">
                ...
                <h2 className={`text-4xl sm:text-5xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                  Master Code with Real
                  <span className="block bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text text-transparent">
                    Interview Problems
                  </span>
                </h2>
                <p className={`text-xl max-w-3xl mx-auto ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  500+ carefully curated problems with our advanced IDE featuring real-time compilation, multiple languages, and detailed performance metrics
                </p>
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-8 mb-16">
                <div className="lg:col-span-2">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {problemCategories.map((category, index) => (
                      <div 
                        key={index}
                        className="problems-card p-6 rounded-xl transition-all duration-300 cursor-pointer hover:scale-105 group"
                        style={{
                          background: isDarkMode
                ...
                <div>
                  <h3 className={`text-2xl font-semibold mb-8 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>🔥 Trending</h3>
                  <div className="space-y-4">
                    {recentProblems.map((problem, index) => (
                      <div 
                        key={index}
                        className="problems-card p-6 rounded-xl transition-all duration-300 cursor-pointer hover:shadow-lg group"
                        style={{
                          background: isDarkMode
                            ? `linear-gradient(135deg, rgba(255, 255, 255, 0.1) 0%, rgba(255, 255, 255, 0.05) 100%)`
                            : `linear-gradient(135deg, rgba(0, 0, 0, 0.02) 0%, rgba(0, 0, 0, 0.01) 100%)`,
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-3 py-2 rounded-lg flex items-center justify-center space-x-2 transition-all font-semibold text-sm"
                        >
              ...
              <div className="text-center">
                <button className="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white px-8 py-4 rounded-xl transition-all text-lg font-bold shadow-lg hover:shadow-xl hover:scale-105">
                  📂 Explore All {problemCategories.reduce((sum, c) => sum + c.problems, 0)}+ Problems
                </button>
              </div>
            </div>
          </div>
